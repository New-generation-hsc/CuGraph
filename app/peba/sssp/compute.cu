                            ...
                            )
{
    ...
    if(tid < num_edges && active_degrees[srcIndex[tid]] > 0){
        if(values[destIndex[tid]] > values[srcIndex[tid]] + weights[tid]){
            *lock = true;
            atomicMin(values + destIndex[tid], values[srcIndex[tid]] + weights[tid]);
...
void graph_problem<uint>::execute(graph_structure<uint> &graph, config_t *conf)
{
    ...
    buffer<uint, DEVICE> dev_weights;
    dev_weights = graph.weights;
