...
int main(int argc, char *argv[]){
    ...
    FILE *fp = open_file_access(conf.input_path, "r");
     ...
     /* construct csr graph representation */
    csr_graph csr;
    parse_graph(fp, csr, conf.undirected, true, conf.verbose);

    graph_structure<float> graph;

    graph.graph_construct(csr);

    graph_problem<float> pagerank;

    pagerank.execute(graph, &conf);
