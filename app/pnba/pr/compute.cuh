                            ...
                            )
{
    ...
    if(tid < nodes){
        ...
        for(uint eid = start_addr; eid < end_addr; ++eid){
            ...
            tmp += values[src] / neighbors[eid];
        }

        tmp = tmp * factor + (1.0 - factor) / nodes;

        if(fabs(tmp - values[tid]) >= threshold){
            *lock = true;
            values[tid] = tmp;
        }
    }
            ...
            )
{
    ...
    buffer<uint, DEVICE> dev_neighbors( csr.m );
    dev_neighbors = csr.neighbors;
